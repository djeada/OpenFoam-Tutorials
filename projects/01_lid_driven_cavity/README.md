...
Since icoFoam is an incompressible solver, density does not appear explicitly. The pressure field `p` has dimensions of m²/s² (kinematic pressure).
...
| Property | Value |
|---|---|
| Domain size (x × y × z) | 0.1 m × 0.1 m × 0.01 m |
| Cell count | 20 × 20 × 1 = 400 cells |
| Cell type | Hexahedral (structured) |
| Grading | Uniform (simpleGrading 1 1 1) |
| Cell size (Δx × Δy) | 0.005 m × 0.005 m |
...
**CFL number estimate:** With Δx = 0.005 m and U_max = 1 m/s:

```
CFL = U × Δt / Δx = 1 × 0.001 / 0.005 = 0.2
```
...
- Double `deltaT` to 0.002 (CFL ≈ 0.4): Should still be stable.
- Try `deltaT = 0.005` (CFL ≈ 1.0): At the stability boundary.
- Try `deltaT = 0.01` (CFL ≈ 2.0): Likely diverges — observe the behavior.
