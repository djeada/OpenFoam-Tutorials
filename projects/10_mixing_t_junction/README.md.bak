...
(Re ≈ 2300 for pipe flow), justifying the use of a turbulence model.
...
Good mixing requires L_mix ≈ 10–20 hydraulic diameters downstream
...
transport. The k-ε model provides ν_t, and `alphat` provides the
...
| Block | Region | Cells (x × y × z) | Dimensions (m) |
|---|---|---|---|
| 0 | Main pipe, left of junction | 50 × 40 × 1 | 0.125 × 0.1 × 0.01 |
| 1 | Main pipe, junction zone | 20 × 40 × 1 | 0.05 × 0.1 × 0.01 |
| 2 | Main pipe, right of junction | 130 × 40 × 1 | 0.325 × 0.1 × 0.01 |
| 3 | Branch pipe | 20 × 40 × 1 | 0.05 × 0.1 × 0.01 |

**Total cells:** 50×40 + 20×40 + 130×40 + 20×40 = 2000 + 800 + 5200 + 800
= **8,800 cells**
...
| Patch | Type | Value | Notes |
|---|---|---|---|
| All patches | `calculated` | `0` | Derived from `p_rgh` + ρgh |
| `frontAndBack` | `empty` | — | 2D constraint |
...
| Patch | Type | Value | Notes |
|---|---|---|---|
| `mainInlet` | `fixedValue` | `0.06` m²/s² | TI ≈ 3.5% at 1.0 m/s |
| `branchInlet` | `fixedValue` | `0.015` m²/s² | TI ≈ 3.5% at 0.5 m/s |
...
| Patch | Type | Value | Notes |
|---|---|---|---|
| `mainInlet` | `fixedValue` | `0.0495` m²/s³ | Estimated from k and L |
| `branchInlet` | `fixedValue` | `0.00877` m²/s³ | Estimated from k and L |
| `outlet` | `zeroGradient` | — | Free outflow |
| `walls` | `epsilonWallFunction` | `0.0495` | Wall function for ε |
| `frontAndBack` | `empty` | — | 2D constraint |
...
| Patch | Type | Value | Notes |
|---|---|---|---|
| `mainInlet` | `calculated` | `0` | Computed from k and ε |
| `branchInlet` | `calculated` | `0` | Computed from k and ε |
| `outlet` | `calculated` | `0` | Computed from k and ε |
| `walls` | `nutkWallFunction` | `0` | Standard wall function for ν_t |
| `frontAndBack` | `empty` | — | 2D constraint |
...
| Patch | Type | Value | Notes |
|---|---|---|---|
| `mainInlet` | `calculated` | `0` | Computed from ν_t and Pr_t |
| `branchInlet` | `calculated` | `0` | Computed from ν_t and Pr_t |
| `outlet` | `calculated` | `0` | Computed from ν_t and Pr_t |
| `walls` | `compressible::alphatWallFunction` | `0` | Wall function for α_t |
| `frontAndBack` | `empty` | — | 2D constraint |
...
| `equationOfState` | `perfectGas` | Ideal gas law (ρ = p/(RT)) |
...
| Kinematic viscosity | ν | 1×10⁻⁶ | [0 2 -1 0 0 0 0] m²/s |
...
### k-ε Model Constants (Defaults)

| Constant | Value | Description |
|---|---|---|
| Cμ | 0.09 | Eddy viscosity coefficient |
...
The k-ε model uses **standard wall functions** for near-wall treatment:

- `kqRWallFunction` — applies the equilibrium assumption for k at walls
- `epsilonWallFunction` — computes ε from the equilibrium log-law
- `nutkWallFunction` — computes ν_t from k and the wall distance
- `compressible::alphatWallFunction` — wall function for α_t
...
|---|---|---|---|---|
| `p_rgh` | `GAMG` | `GaussSeidel` | 1×10⁻⁷ | 0.01 |
| `U`, `h`, `k`, `epsilon` | `PBiCGStab` | `DILU` | 1×10⁻⁷ | 0.1 |
...
All fields converge to 1×10⁻⁴. The solver stops when all residuals drop
...
| Case | T_main (K) | T_branch (K) | ΔT (K) |
|---|---|---|---|
| Small ΔT | 320 | 300 | 20 |
| Baseline | 350 | 300 | 50 |
| Large ΔT | 400 | 300 | 100 |
...
### Exercise 3: Switch to k-ω SST Model

Replace the k-ε model with k-ω SST for better near-wall behaviour:
...
5. **k-ε turbulence model** — Launder, B.E. and Spalding, D.B. (1974),
