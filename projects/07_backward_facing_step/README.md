...
| Inlet velocity         | $U_\infty$         | 10 m/s                        |
| Kinematic viscosity    | $\nu$          | 1.5 $\times$ 10$^{-5}$ m$^2$/s (air)       |
| Reynolds number        | Re_H       | $\approx$ 8,467 (based on H)         |
...
| Expected reattachment  | Xr/H       | $\approx$ 6.0 $\pm$ 0.5                  |
...
> Re_H $\approx$ 8,467 based on step height is common; using the full downstream channel
...
where $\nu ₜ$ is the turbulent eddy viscosity and k is the turbulent kinetic energy.
...
> **Key insight:** Standard k-$\varepsilon$ consistently under-predicts the reattachment length
> by 15–25%, while k-$\omega$ SST typically predicts Xr/H within 5–10% of experimental
> values. This is one of the primary reasons k-$\omega$ SST became the industry standard
> for separated flows.

### Why k-omega SST for This Problem

The k-$\omega$ SST (Shear Stress Transport) model by Menter (1994) was specifically developed
...
3. **Shear stress limiter** — The SST modification limits the eddy viscosity using
   ...
   to the turbulent kinetic energy: `τ = ρa₁k` where a$_1$ = 0.31. This prevents
   over-prediction of eddy viscosity in adverse pressure gradient flows, which is
   the primary failure mode of standard k-$\varepsilon$ in separated flows.

4. **Blending function** — A smooth blending function F$_1$ transitions between the
...
> sensitivity study varying I from 1% to 10% typically shifts Xr/H by $\pm$0.5.
...
| `constant/transport..` | Newtonian fluid, $\nu$ = 1.5e-5 m$^2$/s (air at 20°C)           |
...
|-------|---------------------|--------|-----------|-----------------------------|
| 0     | Upstream channel    | 4H$\times$1H  | 80 $\times$ 40  | x: 0.25 (refine at step)   |
| 1     | Downstream top      | 30H$\times$1H | 300 $\times$ 40 | x: 4.0 (expand downstream) |
| 2     | Downstream bottom   | 30H$\times$1H | 300 $\times$ 40 | x: 4.0 (expand downstream) |
...
y+ $\approx$ 30–50, appropriate for wall-function treatment. For resolved boundary layers
(y+ < 1), the mesh would need ~10$\times$ more cells in the wall-normal direction.
...
| outlet       | fixedValue  | 0 m$^2$/s$^2$         | Reference pressure           |
...
| Patch        | Type              | Value       | Notes                      |
|--------------|-------------------|-------------|----------------------------|
| inlet        | fixedValue        | 0.24 m$^2$/s$^2$  | ~4% turbulence intensity   |
| outlet       | zeroGradient      | —           | Convective outflow         |
...
| Patch        | Type                | Value       | Notes                    |
|--------------|---------------------|-------------|--------------------------|
| inlet        | fixedValue          | 440 s$^{-1}$     | From mixing length       |
| outlet       | zeroGradient        | —           | Convective outflow       |
...
- **`omegaWallFunction`** — Sets omega based on the analytical near-wall solution,
  ...
  function for the k-$\omega$ SST model.

- **`nutkWallFunction`** — Computes $\nu ₜ$ from the log-law, providing the correct eddy
...
|-------------|--------|--------------------------------|
| $\alpha k$1         | 0.85   | $\sigma k$ in k-$\omega$ region               |
| $\alpha k$2         | 1.0    | $\sigma k$ in k-$\varepsilon$ region               |
| $\alpha ω$1         | 0.5    | $\sigma ω$ in k-$\omega$ region               |
| $\alpha ω$2         | 0.856  | $\sigma ω$ in k-$\varepsilon$ region               |
| $\beta$1          | 0.075  | $\beta$ in k-$\omega$ region                |
| $\beta$2          | 0.0828 | $\beta$ in k-$\varepsilon$ region                |
| $\beta$Star       | 0.09   | $\beta$* (= C$\mu$)                      |
| $\gamma$1          | 5/9    | $\gamma$ in k-$\omega$ region                |
| $\gamma$2          | 0.44   | $\gamma$ in k-$\varepsilon$ region                |
...
| Density $\rho$ (implicit)  | ~1.2 kg/m$^3$        |
| Temperature           | ~20°C              |
| Fluid                 | Air                |
...
5. **Wall shear stress sign change** — On the bottom wall, $\tau$_w changes from
...
|--------|-------------|---------------|-------------|-------------------|
| Coarse | 40 $\times$ 20    | 150 $\times$ 30      | 9,800       | Quick test        |
...
1. Increase wall-normal cell count by 10$\times$
2. Apply first-cell grading to achieve y+ $\approx$ 0.5
...
4. **Menter, F.R.** (1994). "Two-equation eddy-viscosity turbulence models for
   ...
   — Original k-$\omega$ SST paper. Demonstrates superior performance for backward-facing
