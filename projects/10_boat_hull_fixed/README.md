# 🚢 Project 10 — Fixed Boat Hull in Calm Water
...
- Is the natural precursor to Project 11 (floating hull)
...
| Phases               | Water (ρ=998.2 kg/m³) + Air (ρ=1.225 kg/m³) |
| Flow speed           | 0.3 m/s (towing speed)               |
| Boat length          | 0.155 m (~6 inches)                  |
| Reynolds number      | Re ≈ 46,500 (based on boat length)   |
| Froude number        | Fr ≈ 0.243 (based on boat length)    |
| Turbulence model     | Laminar (first pass)                 |
| Surface tension      | σ = 0.072 N/m                        |
| Gravity              | g = -9.81 m/s² (z-direction)         |

The **Froude number** Fr = U / √(gL) ≈ 0.3 / √(9.81 × 0.155) ≈ 0.243,
...
material properties that vary according to α:
...
Surface tension σ acts at the interface and is modeled using the
...
where κ is the interface curvature estimated from the α field.
...
| Lateral     | ~3W each side    | ±0.25 m     | No wall interference       |
...
- **sides**: `slip` — frictionless lateral boundaries
...
- **inlet/bottom/sides/boat**: `fixedFluxPressure` — adjusts pressure to
  match the velocity boundary condition
...
`setFields` fills everything below z = 0 with water (α = 1).

- **inlet**: `fixedValue 1` — water enters from the left
- **outlet/atmosphere**: `inletOutlet` — prevents air from flowing in as water
- **boat/bottom/sides**: `zeroGradient` — interface meets walls naturally
...
- **Domain**: [-0.5, 1.0] × [-0.25, 0.25] × [-0.15, 0.15]
- **Cells**: 60 × 20 × 12 = 14,400 cells
...
Uses `boxToCell` to set α = 1 in a huge box from (-10,-10,-10) to (10,10,0).
...
1. **Castellated mesh**: Refines cells near the hull surface
   - Surface refinement: levels 2-3 (cells 4-8× smaller than background)
...
| sides      | slip                          | fixedFluxPressure  | zeroGradient       |
...
- **totalPressure**: Sets p₀ = 0 at the outlet/atmosphere, allowing
...
- **inletOutlet for α**: Prevents air from being "sucked in" as water at
  outflow boundaries.
...
After meshing, the entire domain is initialized as air (α = 0). The
...
with α = 1. The box is intentionally oversized — only cells inside the
...
The VOF method solves a transport equation for the phase fraction α:
...
compression only acts at the interface (where 0 < α < 1), not in pure
fluid regions.

### MULES Algorithm

OpenFOAM uses the **Multidimensional Universal Limiter with Explicit
Solution (MULES)** algorithm to solve the α equation. MULES ensures:

1. **Boundedness**: α stays between 0 and 1 (no unphysical overshoot)
...
1. **Volume fraction equation** (α transport):
   ```
   ∂α/∂t + ∇·(Uα) + ∇·(U_c α(1-α)) = 0
   ```
...
- `ρ, μ`: mixture density and viscosity (functions of α)
...
With `adjustTimeStep yes`, interFoam automatically adjusts Δt to satisfy:

- **maxCo = 0.5**: Maximum Courant number for velocity
- **maxAlphaCo = 0.3**: Maximum Courant number for the interface

The interface Courant limit is stricter because the α equation is solved
...
```bash
cd projects/10_boat_hull_fixed
./Allrun
```
...
Sets α = 1 below z = 0 (water region). Check: `log.setFields`
...
set xlabel 'Time (s)';
set ylabel 'Cd';
plot 'postProcessing/forceCoeffs/0/coefficient.dat' u 1:2 w l title 'Drag';
pause -1
"
...
At Fr ≈ 0.24 and Re ≈ 46,500, you should observe:
...
| Drag coefficient   | Cd ≈ 0.1–0.5        |
| Bow wave height    | 1–5 mm              |
| Max Courant number | ≤ 0.5 (controlled)  |
...
**Symptom**: The water surface becomes diffuse (α spreads over many cells).
...
1. Run at U = 0.15 m/s (Fr ≈ 0.12) — minimal wave making
2. Run at U = 0.3 m/s (Fr ≈ 0.24) — moderate waves (baseline)
3. Run at U = 0.6 m/s (Fr ≈ 0.49) — strong waves
4. Plot drag vs. speed — does it follow a U² relationship?
...
This project is the stepping stone to Project 11 (floating hull):
...
5. Proceed to [Project 11](../11_boat_hull_floating/) to see the answer
...
|---------|-------------|------------|
| [07 — Dam Break](../07_dam_break/) | Two-phase dam break simulation | Also uses interFoam/VOF — simpler geometry, same physics |
| [11 — Floating Hull](../11_boat_hull_floating/) | Free-floating boat hull | Next step — adds 6-DOF rigid body motion to this case |
| [03 — NACA Airfoil](../03_naca_airfoil_analysis/) | External aerodynamics | Similar external flow concepts, single-phase |
| [05 — Ahmed Body](../05_ahmed_body_aerodynamics/) | Bluff body aerodynamics | Similar drag analysis concepts |

### Learning Path

```
  ┌─────────────────────┐     ┌──────────────────────────┐
  │  07 — Dam Break     │ ──► │  10 — Fixed Boat Hull    │
  │  (intro to VOF)     │     │  (THIS PROJECT)          │
  └─────────────────────┘     └──────────┬───────────────┘
                                         │
                                         ▼
                              ┌──────────────────────────┐
                              │  11 — Floating Boat Hull  │
                              │  (adds 6-DOF motion)      │
                              └──────────────────────────┘
```
