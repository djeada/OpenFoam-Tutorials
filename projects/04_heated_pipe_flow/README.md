# 04 — Heated Pipe Flow (Conjugate Heat Transfer)
...
| **Reynolds Number** | Re ≈ 3333 (based on hydraulic diameter) |
...
```
∂(ρU)/∂t + ∇·(ρUU) = -∇p + ∇·(μ_eff ∇U) + ρg
```

**Energy (enthalpy form):**

```
∂(ρh)/∂t + ∇·(ρUh) = ∇·(α_eff ∇h) + ∂p/∂t
```

where:
- `ρ` = density (calculated from ideal gas law: ρ = p/(RT))
- `U` = velocity vector
- `p` = pressure
- `h` = specific enthalpy
...
```
L_thermal ≈ 0.05 × Re × Pr × D_h
```

For our case: L_thermal ≈ 0.05 × 6667 × 0.71 × 0.2 ≈ **47 m**
...
| `0/p` | Total pressure | Calculated from `p_rgh` + ρgh |
...
| `constant/g` | Gravity vector | (0, -9.81, 0) m/s² |
| `constant/thermophysicalProperties` | Gas properties | Air: Cp=1005, Pr=0.71 |
| `constant/transportProperties` | Kinematic viscosity | ν = 1.5×10⁻⁵ m²/s |
| `constant/turbulenceProperties` | Turbulence model | k-epsilon (RAS) |
| `system/blockMeshDict` | Mesh definition | 100×20×1 cells, 2m×0.1m×0.01m |
...
| Cells in z | 1 |
| **Total cells** | **2000** |
| Cell aspect ratio | 1:1 (Δx = 0.02 m, Δy = 0.005 m → AR = 4:1) |
...
- The first cell height of Δy = 0.005 m gives an approximate y+ value suitable for
  wall functions (y+ ≈ 30–100 range).
...
- **Inlet:** Fixed turbulence levels representing low-intensity incoming turbulence.
  The turbulence intensity is approximately I = sqrt(2k/3) / U ≈ sqrt(2×0.1/3) / 0.5 ≈ 52%.
...
cd projects/04_heated_pipe_flow
...
```
T_out_bulk ≈ T_in + (2 × h_conv × L × ΔT) / (ṁ × Cp)
```
...
```
Nu_local(x) = q_wall(x) × D_h / (k_thermal × (T_wall - T_bulk(x)))
```
...
- **Question:** How does the bulk outlet temperature change? Is it linear with ΔT?
...
- Remember to adjust `k` and `epsilon` inlet values for consistency:
  - k ≈ 1.5 × (U × I)² where I is turbulence intensity (e.g., 5%)
  - epsilon ≈ C_μ^0.75 × k^1.5 / l, where l is the turbulence length scale
...
- Change the mesh resolution in `system/blockMeshDict`:
  - Coarse: 50×10×1 (500 cells)
  - Medium: 100×20×1 (2000 cells) ← current
  - Fine: 200×40×1 (8000 cells)
  - Very fine: 400×80×1 (32000 cells)
...
- Target y+ ≈ 30–100 for wall functions, or y+ < 1 for resolving the viscous sublayer
...
- Increase x-direction cells to 500 (maintaining Δx ≈ 0.02 m)
...
| [03 — NACA Airfoil](../03_naca_airfoil_analysis/) | External aerodynamics, turbulence | Same k-epsilon model, different application |
