...
| Water viscosity    | 1 $\times$ 10$^{-6}$                | m$^2$/s    |
| Air viscosity      | 1.48 $\times$ 10$^{-5}$             | m$^2$/s    |
| Surface tension    | 0.07                     | N/m     |
| Gravity            | 9.81 (downward)          | m/s$^2$    |
...
where:
- `ρ` — mixture density (varies with $\alpha$)
- `U` — velocity field (shared by both phases)
- `p_rgh` — dynamic pressure (p - $\rho$ g · x), removes the hydrostatic component
...
of the two phases using the volume fraction $\alpha$:
...
| Courant number           | $\leq$ 0.5                 | Higher = may need smaller maxCo     |
...
0.024 $\times$ 0.048 m. This requires splitting the domain into multiple blocks.
...
| Mesh   | Cells    | $\Delta x$ (m)   | Expected Outcome                       |
|--------|----------|----------|----------------------------------------|
| Coarse | 23$\times$23    | 0.0254   | Fast but diffused interface            |
| Medium | 46$\times$46    | 0.0127   | Good balance (this tutorial)           |
| Fine   | 92$\times$92    | 0.0063   | Sharper interface, slower              |
| VFine  | 184$\times$184  | 0.0032   | Detailed splashing features visible    |
...
| $\sigma$ (N/m) | Physical Meaning                | Effect on Flow                       |
|---------|----------------------------------|--------------------------------------|
| 0.0     | No surface tension               | Maximum interface fragmentation      |
| 0.07    | Water-air (standard)             | Slight interface smoothing           |
| 0.7     | 10$\times$ enhanced                     | Noticeable rounding of interface     |
| 7.0     | 100$\times$ enhanced                    | Strong surface tension dominance     |
...
3. **Brackbill, J.U., Kothe, D.B. and Zemach, C.** (1992). "A Continuum Method
   ...
   - Converts surface tension to a volume force using $\nabla$$\alpha$
