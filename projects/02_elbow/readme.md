...
shear layer forms. Momentum diffusion (controlled by the kinematic viscosity ν)
...
```
∇·U = 0                             (continuity)
∂U/∂t + ∇·(UU) = −∇p + ν ∇²U       (momentum)
```

where **U** is the velocity vector, **p** is the kinematic pressure (pressure
divided by density, p/ρ), and **ν** is the kinematic viscosity. Because the fluid
...
2. **`icoFoam`** — runs the transient solver for 1500 time steps (75 s at Δt = 0.05).
